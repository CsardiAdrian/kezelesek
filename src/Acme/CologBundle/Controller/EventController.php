...
//  Add Function *****************************/
    public function addEventAction(Request $request)
    {
        ...
        $guest = $this->getDoctrine()->getRepository('AcmeCologBundle:guest')->findAll();
        ...
        if ($addEventForm->isValid()){
            ...
            $gid = $_POST['guest'];
            $guestId = $this->getDoctrine()->getRepository('AcmeCologBundle:guest')->find($gid);
...
//  Add Success Function *****************************/
    public function addEventSuccessAction()
    {
        ...
        if ($securityContext->isGranted('IS_AUTHENTICATED_REMEMBERED')) {
            // authenticated REMEMBERED, FULLY will imply REMEMBERED (NON anonymous)
            return $this->render('AcmeCologBundle:Default:addEventFormSuccess.html.twig');
        }else{
...
//            exit(\Doctrine\Common\Util\Debug::dump($event));
            if ($event) {
                $em->remove($event);
                $em->flush();
                return $this->redirect($this->generateUrl('_deleteEventSuccess'));
            }
            else{
...
//            exit(\Doctrine\Common\Util\Debug::dump($id));
    ...
    public function sendMailAction() {
        ...
        ->setBody($this->renderView('AcmeCologBundle:Default:email.txt.twig', array('name' => 'Adrián')))
